...
pub async fn upsert_many<'e>(
    ...
    let postal_codes: Vec<&str> = unique.iter().map(|p| p.postal_code.as_str()).collect();
    ...
    let rows = sqlx::query!(
        r#"INSERT INTO pharmacies (external_id, provider, name, postal_code, city, address, url, first_seen_at, last_seen_at)
           SELECT u.external_id, u.provider, u.name, u.postal_code, u.city, u.address, u.url, now(), now()
           FROM UNNEST($1::text[], $2::text[], $3::text[], $4::text[], $5::text[], $6::text[], $7::text[])
                AS u(external_id, provider, name, postal_code, city, address, url)
           ON CONFLICT (external_id) DO UPDATE SET
               provider = EXCLUDED.provider, name = EXCLUDED.name, postal_code = EXCLUDED.postal_code, city = EXCLUDED.city,
...
pub async fn list<'e>(
    ...
    let rows = sqlx::query!(
        r#"SELECT p.id, p.external_id, p.provider, p.name, p.postal_code, p.city, p.address, p.url, p.first_seen_at, p.last_seen_at,
