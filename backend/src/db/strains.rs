//! `strains`: stable identities keyed by `(name_key, bezeichnung_key)`.
...
pub async fn upsert_many<'e>(
    ...
    for input in inputs {
        let key = (input.name_key.as_str(), input.bezeichnung_key.as_str());
    ...
    let bez_keys: Vec<&str> = unique.iter().map(|s| s.bezeichnung_key.as_str()).collect();
    let names: Vec<&str> = unique.iter().map(|s| s.name.as_str()).collect();
    let bezs: Vec<&str> = unique.iter().map(|s| s.bezeichnung.as_str()).collect();
    let genetiks: Vec<&str> = unique.iter().map(|s| s.genetik.as_str()).collect();
    ...
    let rows = sqlx::query!(
        r#"INSERT INTO strains (name_key, bezeichnung_key, name, bezeichnung, genetik, thc_label, cbd_label, first_seen_at, last_seen_at)
    ...
    Ok(rows
        .into_iter()
        .map(|r| ((r.name_key, r.bezeichnung_key), r.id))
        .collect())
}

pub async fn get<'e>(exec: impl PgExecutor<'e>, id: i64) -> sqlx::Result<Option<StrainRow>> {
    let row = sqlx::query!(
        r#"SELECT id, name, bezeichnung, genetik, thc_label, cbd_label, first_seen_at, last_seen_at,
